...
| `scorchkit-core` | targets, findings, evidence, results, events, correlation, CVE, and compliance types | CLI, MCP, storage, or agent adapters |
...
The family, storage, MCP, CLI, and agent packages currently have no dependency on another internal
ScorchKit package. No lower package may depend on the root `scorchkit` package. Exact allowed edges
are enforced in `tests/workspace_architecture.rs`.
