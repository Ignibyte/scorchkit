...
`CveMatchModule::new(lookup: Box<dyn CveLookup>)` takes its backend by injection — it's intentionally absent from `infra::register_modules()`. The fixture-backed `infra::MockCveLookup` exercises the module in tests; the production `infra::cve_nvd::NvdCveLookup` (WORK-103b) ships a real NIST NVD 2.0 client behind the same trait. Backend selection is config-driven: `[cve] backend = "disabled" | "mock" | "nvd"` plus a `[cve.nvd]` sub-block (`api_key`, `base_url`, `cache_dir`, `cache_ttl_secs`). `infra::cve_lookup::build_cve_lookup(&AppConfig)` is the factory; `Engine::infra_scan` (which `full_assessment` calls) consults it and appends `CveMatchModule` automatically when a backend is configured. The NVD client owns its own `reqwest::Client` (separate from the scan client so pen-test proxy / insecure-TLS settings never leak into vendor calls), wraps a `governor::RateLimiter` sized to NVD's published quotas (5 req/30s anonymous, 50 req/30s with key), and consults a sha256-keyed file-system TTL cache (`infra::cve_cache::FsCache`, with negative caching). See `docs/modules/cve-nvd.md` for the operator-facing config reference.
