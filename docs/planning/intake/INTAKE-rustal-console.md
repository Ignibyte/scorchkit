...
created: 2026-08-21
ticket: TICKET-033
pipeline_spec: docs/planning/pipeline/active/rustal-console.spec.md
---
...
A full ScorchKit suite needs durable navigation, live job status, evidence drill-down, triage,
read-only engagement posture, and target registration beyond compact conversation components.
Rustal already provides a Rust web framework with compiled pages, modules, authentication/RBAC,
audit, PostgreSQL support, and server-sent updates, but ScorchKit core must remain usable without it.
...
jobs, or storage. The active engagement remains immutable authority in v1; the console edits
registered targets and per-run restrictions rather than presenting project membership as an
engagement grant.
...
| REQ-003 | When findings are inspected or triaged, the console shall display scanner evidence, model analysis, transition history, suppressions, correlations, and degraded coverage as distinct data. | Browser snapshots and cross-surface parity tests. |
| REQ-004 | When an operator changes target registration or per-run restrictions, starts or cancels work, or changes triage state, the console shall call an authenticated API command and shall not infer permission from a rendered page, session label, or project membership. | Policy, CSRF, spoofing, denial, and audit tests. |
