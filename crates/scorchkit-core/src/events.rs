...
//! ```no_run
//! # async fn example() {
//! use scorchkit_core::engine::events::{EventBus, ScanEvent};
//!
...
/// the composition layer's `AuditLogHandler`) can emit events as JSON.
...
/// ```no_run
/// # use std::sync::Arc;
/// # use scorchkit_core::engine::events::{EventBus, EventHandler, ScanEvent, subscribe_filtered};
/// # use scorchkit_core::engine::severity::Severity;
