...
//! vulnerabilities.
//!
//! A fixture-backed `MockCveLookup` and the infra-side `CveMatchModule` live behind the
...
pub struct CveRecord {
    ...
    /// `CVE-YYYY-NNNN` id. Populated by the infrastructure `OsvCveLookup`;
    /// empty for NVD and `MockCveLookup`.
    ///
    /// Consumed by the infrastructure `MultiCveLookup` for
...
pub trait CveLookup: Send + Sync {
    ...
    /// # Errors
    ///
    /// Implementations return [`crate::error::ScorchError`]
