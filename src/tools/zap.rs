...
impl ScanModule for ZapModule {
    ...
    async fn run(&self, ctx: &ScanContext) -> Result<Vec<Finding>> {
        ...
        parse_zap_output(&output.stdout, target)
    }
}

fn parse_zap_output(output: &str, target_url: &str) -> Result<Vec<Finding>> {
    ...
    if let Some(alerts) = json["alerts"].as_array().or_else(|| {
        ...
        for alert in alerts {
            let name = alert["name"].as_str().or(alert["alert"].as_str()).unwrap_or("ZAP Finding");
            let desc = alert["desc"].as_str().or(alert["description"].as_str()).unwrap_or("");
            let risk = alert["riskcode"].as_str().or(alert["risk"].as_str()).unwrap_or("0");
            let url = alert["url"].as_str().unwrap_or(target_url);
            let solution = alert["solution"].as_str();
            let cweid = alert["cweid"].as_str().or(alert["cwe"].as_str());
