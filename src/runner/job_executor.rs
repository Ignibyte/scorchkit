//! Shared bounded execution for scanner-family jobs.
//!
//! The executor owns scheduling, cancellation, a whole-batch wall-time budget, and deterministic
//! outcome ordering. It deliberately knows nothing about targets, policy, findings, events,
//! persistence, terminal output, or agent providers. Family orchestrators keep those concerns and
//! pass policy-sealed module futures through this boundary.
...
pub(crate) async fn cancel_on_token<F, T>(cancellation: &CancellationToken, future: F) -> Result<T>
where
